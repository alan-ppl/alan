...
def get_model(data, covariates):
    ...
    def joint_logdensity(params, data, covariates):
        ...
        z_non_cent = stats.norm.logpdf(params.z, 1/100 *params.mu_z, 1/100 *jnp.exp(params.psi_z)).sum()
        #z = params.mu_z + params.z_non_cent * jnp.exp(params.psi_z)

        obs = stats.bernoulli.logpmf(data, jax.nn.softmax((100 *params.z * covariates['x'].transpose(1,0,2)).sum(-1)).transpose()).sum()
