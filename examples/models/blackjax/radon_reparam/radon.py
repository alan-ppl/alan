...
def get_model(data, covariates):
    ...
    def joint_logdensity(params, data, covariates):
        ...
        #County level
        County_mean = stats.norm.logpdf(params.County_mean, 1/100 *params.State_mean[:,jnp.newaxis], 1/100 *jnp.exp(params.State_log_sigma)[...,jnp.newaxis]).sum()
        #County_mean = State_mean[:,jnp.newaxis] + params.County_mean_non_cent * jnp.exp(params.State_log_sigma)[...,jnp.newaxis]
        Beta_u = stats.norm.logpdf(params.Beta_u, 0., 1/10).sum()
        Beta_basement = stats.norm.logpdf(params.Beta_basement, 0., 1/1000).sum()
        County_log_sigma = stats.norm.logpdf(params.County_log_sigma, 0., 1.).sum()
        #Zip level
        obs = stats.norm.logpdf(data.reshape(5,7,5), 100*County_mean + covariates['basement'].reshape(5,7,5)*params.Beta_basement*1000 + covariates['log_uranium'].reshape(5,7,5) * params.Beta_u*10, jnp.exp(params.County_log_sigma)).sum()
