...
ALL_MODEL_NAMES = ['bus_breakdown_reparam', 'movielens_reparam']
...
def plot_method_K_lines(ax, 
    ...
    for i in range(num_lrs):
        if i < metric.shape[0]:
            ...
            ax.plot(xs, smoothed_metric, label=f'{method.upper()}: lr={lrs[i]}', color=colour, alpha=alpha_val)
        ...
        for k, K in enumerate(Ks_to_plot[model_name]):
            for j, method_name in enumerate(results[model_name].keys()):
                ...
                plot_method_K_lines(ax = axs[0,col_counter], 
                                    ...
                                    num_lrs = num_lrs,
                                    colour = colour,
                                    y_lim= elbo_ylims_per_K[model_name][K],
                                    x_axis_iters = x_axis_iters,
                                    smoothing_window = smoothing_window,
                                    error_bars = error_bars,
                                    alpha_func = alpha_func)
                ...
                plot_method_K_lines(ax = axs[1,col_counter], 
                                    ...
                                    num_lrs = num_lrs,
                                    colour = colour,
                                    y_lim= pll_ylims_per_K[model_name][K],
                                    x_axis_iters = x_axis_iters,
                                    smoothing_window = smoothing_window,
                                    error_bars = error_bars,
                                    alpha_func = alpha_func)
...
if __name__ == "__main__":
    ...
    best_Ks = {'bus_breakdown_reparam': [30], 'movielens_reparam': [30]}
