...
def get_data():
    data = pd.read_csv('../bus_breakdown/Bus_Breakdown_and_Delays.csv', header=0, encoding='latin-1')
...
for i in range(1):
    ...
    I = df['Busbreakdown_ID'].nunique()  # 144765
    J = df['Boro'].nunique()             # 11
    M = df['School_Year'].nunique()      # 8
    ...
    run_type = np.broadcast_to(run_type, (M,J,I,run_type.shape[-1]))

    # bus_company_name = Bus_Company_Name.reshape(M, J, I, -1)
    bus_company_name = np.broadcast_to(Bus_Company_Name, (M,J,I,Bus_Company_Name.shape[-1]))

    # delay = delay.reshape(M, J, I)
    delay = np.broadcast_to(delay, (M,J,I))
