...
for j in range(1,4):
    for i in range(len(comp_modes)):
        ...
        with open(f'mem_usage_plate_{j}.pkl', 'rb') as f:
            mem_usage = pickle.load(f)
        with open(f'time_usage_plate_{j}.pkl', 'rb') as f:
            time_usage = pickle.load(f)
        #Plot surface defined by numpy arrays
        # if mode == 'no_checkpoint':
        #     ax.set_title(f"No checkpointing")
        # else:
        ax.set_title(f"Split=10 on plate {j}")
        ...
        plt.savefig('mem_usage_{}_plate_{}.png'.format(mode,j))
