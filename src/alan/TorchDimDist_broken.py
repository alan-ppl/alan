...
class TorchDimDist():
    ...
    def __init__(self, dist, **kwargs):
        ...
        # [batch_shape, all_arg_dims, event_shape] not that all_arg_dims is shared for all args, while the batch shape could be different.

        self.kwargs_tensor = {}
        for name, arg_torchdim in self.kwargs_torchdim.items():
            dims = [*colons(self.arg_batch_ndim[name]), *self.all_arg_dims, *colons(self.arg_event_ndim[name])]
            #[batch_shape, all_arg_dims, event_shape]
            self.kwargs_tensor[name] = singleton_order(arg_torchdim, dims)
