...
from alan_simplified.utils import generic_dims, generic_order, generic_getitem

from model1 import tp as model1
from bernoulli_no_plate import tp as bernoulli_no_plate

tps = [model1, bernoulli_no_plate]
args = itertools.product(tps, sampling_types)


@pytest.mark.parametrize("tp,sampling_type", args)
def test_moments_sample_marginal(tp, sampling_type):
    """
    ...
    sample = tp.problem.sample(K=3, reparam=False, sampling_type=sampling_type)
...
#def test_moments_importance_sample(tp, sampling_type):
#    """
...
#        
#        assert is_moment < marginal_moment + tp.stderrs * stderr
#        assert marginal_moment - tp.stderrs * stderr < is_moment
#
